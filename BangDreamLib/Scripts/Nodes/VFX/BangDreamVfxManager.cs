...
public partial class BangDreamVfxManager : Control
{
    ...
    private readonly Dictionary<NBangDreamVfx, List<(StringName signal, Callable callable)>> _connectedCallables = new();
    ...
    {
        return Callable.From<VfxContext>(context =>
        {
            _ = SafeInvokeAsync(asyncAction, context);
        });
    }
